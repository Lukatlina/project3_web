...
function savePost() {
  ...
  xhr.open("POST", BASE_PATH + "/back/feature/feed/post_create_process.php", true);
...
function openModifyPostModal(board_number) {
  ...
  var xhr = new XMLHttpRequest();
  xhr.open("POST", BASE_PATH + "/back/feature/feed/get_for_modify.php", true);
...
function saveModifiedPost() {
  ...
  var xhr = new XMLHttpRequest();
  xhr.open("POST", BASE_PATH + "/back/feature/feed/post_update_process.php", true);
...
function saveTemporarySaveFile(files) {
  ...
  var xhr = new XMLHttpRequest();
  xhr.open("POST", "/back/feature/media/media_upload_temp.php", true);
...
var scrolled = false;
  ...
  if ((window.innerHeight + window.scrollY) >= height && scrolled === false) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", BASE_PATH + "/back/feature/feed/load_feed.php", true);
    xhr.onreadystatechange = function () {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          console.log("POST 요청 성공");
...
textPreviewDiv.onclick = function() {
    location.href = BASE_PATH + 'post_detail_page.php?board_number=' + post.id;
};
